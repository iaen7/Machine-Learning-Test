# -*- coding: utf-8 -*-
"""
Created on Fri May 19 09:41:00 2017
...
if __name__ == "__main__":
    ...
    if len(queryAddress):
        savePath = input("检测到有该城市数据，请输入即将保存天气数据的路径（如若不输入，将默认保存到h:/Machine Learning Test/"+addressName+".xls）:\n")
        if not savePath.strip():
            if not os.path.exists('h:/Machine Learning Test'):
                os.makedirs('h:/Machine Learning Test')
            savePath = "h:/Machine Learning Test/"+addressName+".xls"
